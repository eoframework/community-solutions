#------------------------------------------------------------------------------
# Amatra Agentic Pre-Sales Platform — DR Environment
# Solution: amatra-presales-platform | Region: us-west-2
# Opportunity: OPP-2026-001 | DR mirrors production sizing per SOW
#------------------------------------------------------------------------------
...
# KMS CMK — separate CMK per region for DR
module "kms" {
  source = "../../modules/aws/kms"

  name_prefix     = local.name_prefix
  description     = "CMK for Amatra platform S3 artifacts and CloudWatch Logs — DR"
  enable_rotation = true
  common_tags     = local.common_tags
}

# VPC — matches production CIDR per configuration.csv DR column
module "networking" {
...
#===============================================================================
# CORE SOLUTION — DR warm-standby platform components
#===============================================================================

# Cognito — DR mirrors production pool configuration
module "cognito" {
...
# DynamoDB — production sizing with PITR for DR RTO/RPO compliance
module "database" {
...
# S3 — production retention, versioning for 0-hour RPO on artifacts
module "storage" {
...
# ECR — DR holds same images for agent failover
module "ecr" {
...
# Secrets Manager — DR-scoped secrets matching production paths
module "secrets" {
...
# Lambda — production-grade concurrency for DR failover
module "compute" {
...
# API Gateway — production throttle limits for DR warm standby
module "api_gateway" {
...
# Step Functions — DR warm standby, same execution graph
...
# GuardDuty — DR operates independently per SOW compliance baseline
module "guardduty" {
...
# CloudTrail — independent audit trail for DR region
module "cloudtrail" {
...
# CodePipeline — DR has own pipeline for image builds
module "cicd" {
...
#===============================================================================
# OPERATIONS — Monitoring (production-grade for DR visibility)
#===============================================================================
...
resource "aws_cloudwatch_metric_alarm" "lambda_error_rate_high" {
  ...
  alarm_description   = "Lambda error rate exceeded threshold — DR environment"
...
resource "aws_cloudwatch_metric_alarm" "stepfunctions_failure_high" {
  ...
  alarm_description   = "Step Functions execution failures exceeded threshold — DR environment"
  alarm_actions       = [module.monitoring.sns_topic_arn]
  ok_actions          = [module.monitoring.sns_topic_arn]
  treat_missing_data  = "notBreaching"
  dimensions = {
