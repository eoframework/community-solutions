#------------------------------------------------------------------------------
# Amatra Agentic Pre-Sales Platform — Test Environment
# Solution: amatra-presales-platform | Region: us-west-2
# Opportunity: OPP-2026-001 | Cost-optimised dev/test configuration
#------------------------------------------------------------------------------
...
# KMS CMK — encryption at rest for test environment
module "kms" {
  source = "../../modules/aws/kms"

  name_prefix     = local.name_prefix
  description     = "CMK for Amatra platform S3 artifacts and CloudWatch Logs — Test"
  enable_rotation = true
  common_tags     = local.common_tags
}

# VPC, private subnets (3 AZs), single NAT Gateway — minimal cost for test
module "networking" {
...
# Cognito User Pool — dev user pool, debug logging
module "cognito" {
...
# DynamoDB tables — reduced TTL and no PITR for test cost savings
module "database" {
...
# S3 artifact bucket — shorter retention, no Glacier transition in test
module "storage" {
...
# ECR repository — same repo, minimal image retention
module "ecr" {
...
# Secrets Manager — same secret paths, test values populated post-deploy
module "secrets" {
...
# Lambda — reduced memory (512 MB) and concurrency for test
module "compute" {
...
# API Gateway HTTP API v2 — lower throttle limits for test
module "api_gateway" {
...
# Step Functions — same state machine, test quotas enforced
...
# GuardDuty — enabled in test per configuration.csv (true for all envs)
module "guardduty" {
...
# CloudTrail — shorter retention in test (30 days)
module "cloudtrail" {
...
# CodePipeline — CI gate validates terraform on every PR
module "cicd" {
...
#===============================================================================
# OPERATIONS — Monitoring (reduced alarms for test)
#===============================================================================
...
resource "aws_cloudwatch_metric_alarm" "lambda_error_rate_high" {
  ...
  alarm_description   = "Lambda error rate exceeded threshold — test environment"
