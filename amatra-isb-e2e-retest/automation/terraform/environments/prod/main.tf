#------------------------------------------------------------------------------
# AWS Agentic Pre-Sales Orchestration Platform — Production Environment
# EO Framework | Solution: aws-agentic-presales | Environment: production
#------------------------------------------------------------------------------
...
module "security" {
  ...
  security = {
    ...
    securityhub_enabled                 = var.security.securityhub_enabled
  }
...
# Wire API Gateway → Generation Initiator Lambda
...
resource "aws_apigatewayv2_route" "post_solutions" {
  ...
  depends_on = [aws_apigatewayv2_integration.generation_initiator]
}

resource "aws_lambda_permission" "generation_initiator_apigw" {
  statement_id  = "AllowAPIGWInvokeGenerationInitiator"
...
# Wire API Gateway → API Handler Lambda (10 routes)
...
resource "aws_apigatewayv2_route" "get_artifact" {
  api_id             = module.api.api_id
  route_key          = "GET /api/v1/solutions/{solution_id}/artifacts/{artifact_type}"
  target             = "integrations/${aws_apigatewayv2_integration.api_handler.id}"
  authorization_type = "JWT"
  authorizer_id      = module.api.authorizer_id
}
...
resource "aws_apigatewayv2_route" "post_admin_quota_override" {
  api_id             = module.api.api_id
  route_key          = "POST /api/v1/admin/quota-override"
  target             = "integrations/${aws_apigatewayv2_integration.api_handler.id}"
  authorization_type = "JWT"
  authorizer_id      = module.api.authorizer_id
}
...
resource "aws_apigatewayv2_route" "post_admin_suspend_user" {
  api_id             = module.api.api_id
  route_key          = "POST /api/v1/admin/users/{user_id}/suspend"
  target             = "integrations/${aws_apigatewayv2_integration.api_handler.id}"
  authorization_type = "JWT"
  authorizer_id      = module.api.authorizer_id
}
...
resource "aws_lambda_permission" "api_handler_apigw" {
  statement_id  = "AllowAPIGWInvokeApiHandler"
  action        = "lambda:InvokeFunction"
...
# DynamoDB throttle alarm (database → monitoring cross-module)
resource "aws_cloudwatch_metric_alarm" "dynamodb_throttle" {
  alarm_name          = "${local.name_prefix}-alarm-dynamodb-throttle"
  alarm_description   = "DynamoDB throttled requests on quota enforcement table"
