#!/usr/bin/env bash
#------------------------------------------------------------------------------
# AWS Cloud Governance Platform — DR Deployment Wrapper
...
echo -e "${BLUE}🚀 ${ENVIRONMENT^^} Environment — AWS Cloud Governance Platform (ap-southeast-4)${NC}"
echo -e "${CYAN}═══════════════════════════════════════════════════════════${NC}"
...
case $COMMAND in
  "init")
    echo -e "${YELLOW}⚙️  Initialising DR Terraform backend (ap-southeast-4)...${NC}"
    terraform init "$@"
    ;;
  "plan")
