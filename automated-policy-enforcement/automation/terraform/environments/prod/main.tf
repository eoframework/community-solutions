#------------------------------------------------------------------------------
# AWS Cloud Governance Platform — Production Environment
# ap-southeast-2 primary | ap-southeast-4 DR | ISO 27001 aligned
#------------------------------------------------------------------------------
...
#===============================================================================
# FOUNDATION — Core infrastructure that all other modules depend on
#===============================================================================

# KMS — encryption keys for all platform data stores
module "kms" {
...
# Networking — Network Account inspection VPC, subnets, Transit Gateway attachment
module "networking" {
...
# Security — IAM roles, SCPs, IAM Identity Center permission sets
module "security" {
...
#===============================================================================
# CORE SOLUTION — Primary governance platform components
#===============================================================================

# Storage — S3 log archive (WORM/Object Lock) and Terraform state bucket
module "storage" {
...
# Database — DynamoDB for AFT workflow state and Terraform state locking
module "database" {
...
# AFT Pipeline — Account Factory for Terraform account vending with ITSM gating
module "aft_pipeline" {
  source = "../../modules/aft"
...
# SIEM Integration — EventBridge + Lambda forwarding Security Hub findings to on-premises SIEM
...
# ITSM Integration — Lambda polling ITSM change-approval for AFT pipeline gating
...
# Config Remediation — Lambda-based auto-remediation for defined low-risk Config findings
...
#===============================================================================
# OPERATIONS — Monitoring, compliance, backup, and DR
#===============================================================================

# Monitoring — CloudWatch dashboards, alarms, and SNS alerting
module "monitoring" {
...
# Best Practices — AWS Config rules (ISO 27001 conformance pack), GuardDuty, AWS Backup
module "best_practices" {
  ...
  dr_region                       = var.solution.dr_region
  guardduty_enabled               = var.monitoring.guardduty_org_detector_enabled
...
# DR Replication — S3 CRR, Config cross-region aggregation, Security Hub cross-region
module "dr" {
  source = "../../modules/dr"
...
# CloudWatch alarm: SIEM DLQ depth — any failed delivery is a P1
resource "aws_cloudwatch_metric_alarm" "siem_dlq_depth" {
  alarm_name          = "${local.name_prefix}-siem-dlq-depth"
  alarm_description   = "P1: SIEM forwarding DLQ has messages — delivery failure detected"
...
# CloudWatch alarm: S3 CRR replication lag — RPO sentinel
resource "aws_cloudwatch_metric_alarm" "crr_replication_lag" {
  alarm_name          = "${local.name_prefix}-crr-replication-lag"
  alarm_description   = "P1: Log Archive S3 CRR lag exceeds RPO threshold (1 hour)"
  comparison_operator = "GreaterThanThreshold"
  ...
  ok_actions          = [module.monitoring.sns_topic_arn]
  dimensions = {
    SourceBucket      = module.storage.log_archive_bucket_name
    DestinationBucket = module.storage.crr_destination_bucket_name
    RuleId            = "log-archive-crr"
  }
  tags = local.common_tags
