#------------------------------------------------------------------------------
# AWS Cloud Governance Platform — Test (Non-Production) Environment
# ap-southeast-2 | debug logging enabled | reduced retention | no console lock
#------------------------------------------------------------------------------
...
# Networking — inspection VPC and VPC endpoints for test environment
module "networking" {
...
# Security — IAM roles and permission sets (SCPs are informational in test)
module "security" {
...
# Storage — S3 log archive and Terraform state bucket (test retention periods)
module "storage" {
...
# Database — DynamoDB for AFT workflow state and state locking (test tables)
module "database" {
...
# AFT Pipeline — Account vending pipeline (ITSM approval required in test too)
module "aft_pipeline" {
  source = "../../modules/aft"
...
# SIEM Integration — Security Hub to SIEM forwarding Lambda (test instance)
...
# ITSM Integration — Change-approval polling Lambda (test instance)
...
# Config Remediation — Auto-remediation Lambda for test validation
...
#===============================================================================
# OPERATIONS — Monitoring, compliance, backup (reduced retention in test)
#===============================================================================

# Monitoring — CloudWatch dashboards and alarms for test environment
module "monitoring" {
...
# Best Practices — Config rules, GuardDuty (optional in test), Backup (short retention)
...
# CloudWatch alarm: SIEM DLQ depth (test — validates pipeline health)
resource "aws_cloudwatch_metric_alarm" "siem_dlq_depth" {
  alarm_name          = "${local.name_prefix}-siem-dlq-depth"
  alarm_description   = "SIEM forwarding DLQ has messages — delivery failure detected (test)"
...
# CloudWatch alarm: AFT pipeline failure (validates pipeline for test accounts)
resource "aws_cloudwatch_metric_alarm" "aft_pipeline_failure" {
  alarm_name          = "${local.name_prefix}-aft-pipeline-failure"
  alarm_description   = "AFT account vending pipeline has failed (test)"
