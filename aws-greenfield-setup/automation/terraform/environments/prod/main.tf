#------------------------------------------------------------------------------
# AWS Multi-Account Landing Zone — Production Environment
# Provider: AWS | Region: us-east-1 (primary) | DR: us-east-2
#------------------------------------------------------------------------------
...
locals {
  ...
  # SCP policy documents — deny root, restrict regions, lock CloudTrail/Config, require MFA
  ...
  permission_sets = {
    platform_admin = {
      ...
      description      = "Full access to platform accounts (Security, Infrastructure, Management)"
      session_duration = "PT${var.sso.session_duration_hours}H"
    }
    workload_admin = {
      name             = "WorkloadAdministratorAccess"
      description      = "Administrative access within specific workload accounts"
      session_duration = "PT${var.sso.session_duration_hours}H"
    }
    security_auditor = {
      name             = "SecurityAuditorAccess"
      description      = "Read-only compliance and security investigation access across all accounts"
      session_duration = "PT${var.sso.session_duration_hours}H"
    }
    network_operator = {
      name             = "NetworkOperatorAccess"
      description      = "Network-specific access scoped to the Network Hub account"
      session_duration = "PT${var.sso.session_duration_hours}H"
    }
    finops = {
      name             = "FinOpsAccess"
      description      = "Billing, Cost Explorer, Budgets read access across all accounts"
      session_duration = "PT${var.sso.session_duration_hours}H"
    }
  }
  ...
  managed_policy_attachments = {
    ...
    workload_admin = {
      permission_set_key = "workload_admin"
      managed_policy_arn = "arn:aws:iam::aws:policy/AdministratorAccess"
    }
    security_auditor_ro = {
...
#===============================================================================
# FOUNDATION — Core governance and security primitives
#===============================================================================
module "governance" {
  ...
  policies = {
    deny_root = {
      ...
      description = "Deny all root account API actions across member accounts"
    ...
    lock_cloudtrail = {
      ...
      description = "Prevent disabling or modifying the CloudTrail organisation trail"
    ...
    rcp_data_perimeter = {
      ...
      description = "Enforce org-boundary data perimeter on all member accounts"
...
#===============================================================================
# CORE SOLUTION — Logging, AFT pipeline infrastructure
#===============================================================================
module "logging" {
...
#===============================================================================
# OPERATIONS — Monitoring, alerting, and cost governance
#===============================================================================
module "monitoring" {
...
# SNS topic policies to allow EventBridge to publish GuardDuty/Security Hub alerts
