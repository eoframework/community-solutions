################################################################################
# MedCore CDS Platform — Production Environment
# Primary region: us-east-1 | Passive DR: us-west-2
################################################################################
...
# KMS CMKs + WAF + GuardDuty + Security Hub + Macie + IAM Access Analyzer
module "security" {
...
# VPC + subnets + NAT Gateways + PrivateLink endpoints
module "networking" {
...
# S3 buckets + Aurora PostgreSQL + ElastiCache Redis
module "storage" {
...
# ECS Fargate + ALBs (clinical dashboard + API backend)
module "compute" {
...
# SageMaker roles + Feature Store + SSM parameters + Model Registry
module "ml_platform" {
...
#===============================================================================
# OPERATIONS — Monitoring, compliance, and observability
#===============================================================================

# CloudWatch dashboards + SNS alarms + AWS Config
module "monitoring" {
...
# WAF WebACL → Dashboard ALB association
...
# WAF WebACL → FHIR inbound ALB association
resource "aws_wafv2_web_acl_association" "fhir_alb" {
  count        = var.security.waf_enabled ? 1 : 0
  resource_arn = module.compute.alb_fhir_arn
  web_acl_arn  = module.security.waf_web_acl_arn

  depends_on = [module.compute, module.security]
}

# Aurora replica lag alarm (database → monitoring)
resource "aws_cloudwatch_metric_alarm" "aurora_replica_lag" {
  ...
  statistic           = "Average"
  threshold           = 5000
  alarm_description   = "HIGH: Aurora replica lag > 5 seconds"
...
# ElastiCache eviction rate alarm (cache → monitoring)
resource "aws_cloudwatch_metric_alarm" "elasticache_evictions" {
  ...
  statistic           = "Sum"
  threshold           = 100
  alarm_description   = "HIGH: ElastiCache eviction rate > 100/min — check cache memory"
  alarm_actions       = [module.monitoring.sns_topic_arn]
  treat_missing_data  = "notBreaching"

  dimensions = {
    ReplicationGroupId = var.cache.elasticache_cluster_id
  }
...
# Kinesis DLQ depth alarm (ingestion → monitoring)
resource "aws_cloudwatch_metric_alarm" "kinesis_dlq_depth" {
  ...
  alarm_description   = "HIGH: Kinesis DLQ has messages — investigate malformed FHIR events"
  alarm_actions       = [module.monitoring.sns_topic_arn]
  treat_missing_data  = "notBreaching"
