#------------------------------------------------------------------------------
# Amatra Agentic Orchestration Platform — Production Environment
...
#===============================================================================
# CORE SOLUTION — DynamoDB, Lambda, API Gateway
#===============================================================================
...
#===============================================================================
# OPERATIONS — Security controls, observability, CI/CD
#===============================================================================
...
module "security" {
  ...
  name_prefix                               = local.name_prefix
  waf_rate_limit_requests_per_ip_per_minute = var.security.waf_rate_limit_requests_per_ip_per_minute
  waf_managed_rules_enabled                 = var.security.waf_managed_rules_enabled
  cloudtrail_enabled                        = var.security.cloudtrail_enabled
  cloudtrail_s3_bucket_name                 = var.security.cloudtrail_s3_bucket_name
  cloudtrail_log_retention_days             = var.monitoring.cloudtrail_log_retention_days
  aws_region                                = var.aws.region
  aws_account_id                            = data.aws_caller_identity.current.account_id
  common_tags                               = local.common_tags
...
module "codebuild" {
  ...
  name_prefix                          = local.name_prefix
  agent_image_project_name             = var.operations.codebuild_agent_image_project_name
  terraform_plan_project_name          = var.operations.codebuild_terraform_plan_project_name
...
resource "aws_wafv2_web_acl_association" "api_gateway" {
  count        = var.security.waf_managed_rules_enabled ? 1 : 0
  resource_arn = module.api_gateway.stage_arn
  web_acl_arn  = module.security.waf_web_acl_arn

  depends_on = [module.api_gateway, module.security]
}

# CloudWatch alarm: Solution Create Lambda error rate
resource "aws_cloudwatch_metric_alarm" "solution_create_errors" {
  ...
  alarm_description   = "Solution Create Lambda error count exceeds threshold"
...
# CloudWatch alarm: GitHub Integration Lambda error rate
resource "aws_cloudwatch_metric_alarm" "github_integration_errors" {
  alarm_name          = "${local.name_prefix}-github-integration-error-rate"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 2
  metric_name         = "Errors"
  namespace           = "AWS/Lambda"
  ...
  statistic           = "Sum"
  threshold           = 1
  alarm_description   = "GitHub Integration Lambda errors — artifacts may not be committed to GitHub"
  treat_missing_data  = "notBreaching"

  dimensions = {
    FunctionName = module.lambda.github_integration_function_name
  }
...
resource "aws_cloudwatch_metric_alarm" "api_gateway_5xx" {
  ...
  depends_on = [module.api_gateway, module.monitoring]
}

# SSM parameter for API Gateway URL (runtime config for Lambda)
