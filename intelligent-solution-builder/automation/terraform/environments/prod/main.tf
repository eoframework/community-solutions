#------------------------------------------------------------------------------
# Amatra Intelligent Solution Builder — Production Environment
# Serverless AI-powered consulting artifact generation platform on AWS
# Region: us-west-2 | Compliance: SOC 2 Type II, GDPR-aligned
#------------------------------------------------------------------------------
...
#===============================================================================
# FOUNDATION — Encryption keys and core security (all other modules depend on these)
#===============================================================================
module "kms" {
...
#===============================================================================
# OPERATIONS — Monitoring, alerting, compliance, and audit
#===============================================================================
module "monitoring" {
...
# WAF WebACL → API Gateway stage association
...
# Lambda → API Gateway integration (submit endpoint)
...
# Lambda → API Gateway integration (status endpoint)
...
# Lambda → API Gateway integration (retrieve endpoint)
...
# Lambda → API Gateway integration (admin endpoint)
...
resource "aws_cloudwatch_metric_alarm" "dlq_depth" {
  ...
  alarm_description   = "P1: DLQ message received — generation job failed after max retries"
...
# Step Functions failure rate alarm → SNS
resource "aws_cloudwatch_metric_alarm" "sfn_failure_rate" {
  alarm_name          = "${local.name_prefix}-sfn-failure-alarm"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 1
  ...
  statistic           = "Sum"
  threshold           = 1
  alarm_description   = "P1: Step Functions execution failures detected in generation workflow"
  alarm_actions       = [module.monitoring.sns_topic_arn]
  treat_missing_data  = "notBreaching"
  dimensions = {
    StateMachineArn = module.orchestration.state_machine_arn
  }
  tags = local.common_tags

  depends_on = [module.monitoring, module.orchestration]
}
